...
- 次のコマンドで、このプロジェクトに必要なパッケージを、インストールします：
  ...
  <details>
    ...
    **Node.jsをインストールしてある状態で、`npm install`コマンドを実行すると、このプロジェクトに必要なライブラリを、一括でインストールできます。**

    - ライブラリインストール後にできること：
      - ローカルサーバーを起動し、ブラウザのでの表示を確認すること（`npm run dev`コマンド実行後、表示されるURLをクリックすれば確認できます）
      - **husky**というライブラリを使い、コミット時に自動でコードの整形・チェックすること（特別な作業はしなくてOK）
    ...
    - その場合、**2通りの対処法**があります：
      ...
      - **Node.js をインストールする。**
        - 様々なインストール方法がありますが、以下のサイトやご自身で調べていただいてそれぞれの好みに合わせてインストールしてください。
